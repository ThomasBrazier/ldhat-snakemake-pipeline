...
figures = summarise.interval.figure(rates.file = mcmc.file, burn.in = burnin, locs.file = locs.file)
```
...
mapfile = paste0(wdirpop, "/ldhat/", dataset, ".", chromosome, ".bpen", bpen, ".res.txt.gz")
...
map = head(map, -1)
# Combine
map = cbind(dataset, chromosome, map)
```


```{r}
ggplot(map, aes(x = end/10^6, y = Mean_rho)) +
        geom_line() +
