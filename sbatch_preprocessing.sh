...
snakemake -s data_preprocessing.snake -p -j 16 --config dataset=${1} --configfile config.yaml --cluster-config cluster.yaml --cluster "sbatch --account={cluster.account}" --use-conda --use-singularity --nolock --rerun-incomplete -F
