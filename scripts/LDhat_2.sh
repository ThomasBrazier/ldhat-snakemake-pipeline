...
wd=/groups/landrec # Working directory

prefix=Hordeum_vulgare_Dreissig2019 # The ID of the dataset
indiv=60 # Size of the population sampled
...
input_data=$wd/data/polymorphism_data/$prefix # Input from data directory
...
# PARALLEL
ncores=5 # Number of cores required on the cluster, passed in argument to the script
...
vcftools --gzvcf $input_data/$prefix.SNP.vcf.gz --out $input/$prefix.filter.SNP --recode --maf $MAF --max-missing $missing
...
vcftools --gzvcf $input/$prefix.filter.SNP.vcf.gz --out $input/$prefix\_$indiv.SNP --keep $input/$prefix\_$indiv.ind --recode
mv $input/$prefix\_$indiv.SNP.recode.vcf $input/$prefix\_$indiv.SNP.vcf
gzip $input/$prefix\_$indiv.SNP.vcf
...
vcftools --gzvcf $input/$prefix\_$indiv.SNP.vcf.gz --out $input/$prefix.chromosome$chrom.SNP --recode --chr $chrom --max-missing-count $indiv
...
# Create pseudo-diploid
sed 's/|.\t.|/|/g' $input/$prefix.chromosome$chrom.phased.vcf > $input/$prefix.chromosome$chrom.hap.vcf
sed -r -i "/^#CHROM/s/\S+//g$indiv" Hordeum_vulgare_Dreissig2019.chromosome1.hap.vcf
bgzip "$input/$prefix.chromosome$chrom.phased.vcf"
...
# Index the vcf
sample=$(sed -n -E '/ERX/p' Hordeum_vulgare_Dreissig2019.chromosome1.hap.vcf)
...
#$wd/ldhat/ldpop/run/ldtable.py -n $N -th $theta -rh $num_rh,$max_rh -s $coal_sizes -t $coal_times --approx --cores $ncore  --log "ldpop.log"
$wd/ldhat/ldpop/run/ldtable.py -n $N -th $theta -rh $num_rh,$max_rh --cores $ncore  --log "$output/ldpop.chromosome$chrom.log" > $output/lk_ldpop.chromosome$chrom.txt
...
vcftools --gzvcf $input/$prefix.chromosome$chrom.phased.vcf.gz --ldhat --chr $chrom --out $output/$prefix.chromosome$chrom

# For haploid correct the indication on the first line of the .sites
sed -e '/\-1/{N;d}' -i $output/$prefix.chromosome$chrom.ldhat.sites
indiv2=$(($indiv*2))
sed "0,/$indiv2/{s//$indiv/}" -i $output/$prefix.chromosome$chrom.ldhat.sites
...
$PathToLDhat/rhomap -seq $prefix.chromosome$chrom.ldhat.sites \
	-loc $prefix.chromosome$chrom.ldhat.locs \
	-lk lk_ldpop.chromosome$chrom.txt \
	-its "$iter" \
	-samp "$samp" \
	-burn "$burn" \
	-prefix "$prefix.chromosome$chrom."
...
$PathToLDhat/stat -input $prefix.chromosome$chrom.rates.txt \
	-burn 5 -loc $prefix.chromosome$chrom.ldhat.locs
...
$PathToLDhot/ldhot --seq $prefix.chromosome$chrom.ldhat.sites \
	--loc $prefix.chromosome$chrom.ldhat.locs \
	--lk lk_ldpop.chromosome$chrom.txt \
	--res res.txt \
	--nsim 1000 \
	--out $prefix.chromosome$chrom

$PathToLDhot/ldhot_summary --res res.txt \
	--hot "$prefix.chromosome$chrom.hotspots.txt" \
	--out "$prefix.chromosome$chrom"
